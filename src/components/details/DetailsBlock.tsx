...
import { AppContext, FLEX_STYLE_ROUNDED } from '../constants.ts';
...
export function DetailsBlock({ children }: DetailsBlockType) {
  ...
  const bgColor = isDefaultTheme ? 'bg-amber-100' : ' bg-gray-800';
  return (
    <div className="flex flex-col flex-1 w-full" onClick={stopEvent}>
      <div
        className={`${FLEX_STYLE_ROUNDED} flex-col w-full h-fit max-h-[70vh] sticky top-0 items-center min-h-fit gap-y-4 ${bgColor}`}
      >
        {children}
      </div>
    </div>
